...
| `warning` | битые блоки, пакеты `NOT_FOUND`, ошибки koji по пакету, ошибки загрузки VEX, отключённый кэш; строки таблицы, где при сбое оставлены прежние koji или VEX, и строки, по которым нечего спрашивать |
