...
| `info` (по умолчанию) | режим; таблица и число строк в ней; при синхронизации — `совпало N, Missing M, новых K`; конфиг (путь или «без конфига»); хаб, тег, RHEL; применённые стримы VEX; число блоков, CVE и пакетов; прогресс загрузки VEX; сводка по `RHEL state` и `SL NVR`; записанные файлы; общее время |
